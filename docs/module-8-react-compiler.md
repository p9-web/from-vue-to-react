---
title: "Module 8 · The React Compiler"
...
learn:
  ...
  outcomes:
    ...
    - "Identify Rules-of-React violations that cause a silent optimization bailout"
  ...
  concepts:
    ...
    - "Rules of React (component purity)"
    - "mutation & side effects during render"
    - "eslint-plugin-react-hooks guardrail"
  misconceptions:
...
# Module 8: The React Compiler — The Eradication of Manual Memoization
...
What it skips is code that **breaks the Rules of React** — the purity contract it relies on to prove an optimization safe. The compiler *assumes* components and Hooks are pure; where they aren't, it declines to memoize that scope:

1. **Mutating props or state during render** — `let { value } = props; value = sanitize(value)` reassigns a binding into what is an immutable snapshot. Derive into a *new* variable instead.
2. **Side effects during render** — `Date.now()`, `Math.random()`, or direct DOM/global mutation (`window.x = …`) make render non-idempotent, so the compiler cannot assume a stable result.

Note what is *not* on this list: optional chaining, nullish coalescing, dynamic property access (`obj[key]`), and `try/catch` are ordinary JavaScript the compiler handles. The boundary is **purity**, not syntax.
...
The guardrail: install **`eslint-plugin-react-hooks@latest`** (v6 folds in the React Compiler's rules and flags Rules-of-React violations even if you haven't enabled the compiler) and elevate those diagnostics to **error** in critical paths, so an un-optimizable component fails CI instead of shipping silently. Treat "the compiler will handle it" as a claim you *lint into truth*, not an assumption.
